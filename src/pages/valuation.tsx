...
export default function Valuation() {
  ...
  const [form, setForm] = useState({ valuationDate:today(), fairMarketValue:'', method:'409A', valuationBy:'', notes:'' })
  ...
  async function save() {
    ...
    setSaving(true)
    const ref = await addDoc(collection(db,'companies',companyId,'valuations'), { ...form, fairMarketValue:parseFloat(form.fairMarketValue), companyId, createdAt:serverTimestamp(), createdBy:user!.uid })
    await logAudit({ companyId, userId:user!.uid, userEmail:profile?.email||'', action:'valuation_updated', entityType:'valuation', entityId:ref.id, entityLabel:`FMV ${form.fairMarketValue} on ${form.valuationDate}`, after:form })
    setValuations(v => [{ id:ref.id, ...form, fairMarketValue:parseFloat(form.fairMarketValue) }, ...v])
    setShowForm(false); setForm({ valuationDate:today(), fairMarketValue:'', method:'409A', valuationBy:'', notes:'' })
    setSaving(false)
  }
    ...
    <Layout title="Valuations">
      <div style={{ maxWidth:800 }}>
        ...
        {showForm && (
          <div className="card mb-4">
            ...
            <div style={{ display:'flex', gap:10 }}>
              <button onClick={save} disabled={saving} className="btn btn-primary">{saving?'Saving...':'Save Valuation'}</button>
        ...
        <div className="card" style={{ padding:0, overflow:'hidden' }}>
          <table className="tbl">
            <thead><tr><th>Date</th><th>FMV / Share</th><th>Method</th><th>Valuated By</th><th>Notes</th></tr></thead>
            <tbody>
              {valuations.length === 0 ? (
                <tr><td colSpan={5} style={{ textAlign:'center', padding:32, color:'var(--text3)' }}>No valuations yet. Add your first 409A.</td></tr>
              ) : valuations.map(v => (
                <tr key={v.id}>
