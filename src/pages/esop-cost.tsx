      ...
      <div style={{ maxWidth:1000 }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:16, marginBottom:24 }}>
          ...
          <div className="card">
            ...
            {Object.entries(fyTotals).sort().map(([fy,cost]) => (
              <div key={fy} style={{ display:'flex', justifyContent:'space-between', marginBottom:6, fontSize:13 }}>
                <span style={{ color:'var(--text2)' }}>{fy}</span>
                <span style={{ fontWeight:700 }}>{fmtC(cost)}</span>
              </div>
            ))}
            {Object.keys(fyTotals).length === 0 && <div style={{ fontSize:12, color:'var(--text3)' }}>Add valuations to see FY allocation</div>}
          </div>
        </div>
        <div className="card" style={{ padding:0, overflow:'hidden' }}>
          <table className="tbl">
            <thead><tr><th>Grant #</th><th>Employee</th><th>Exercise Price</th><th>Grant Date FMV</th><th>Intrinsic Value</th><th>Options</th><th>Total Expense</th></tr></thead>
            <tbody>
              {results.length === 0 ? (
                <tr><td colSpan={7} style={{ textAlign:'center', padding:32, color:'var(--text3)' }}>No grants found. Add valuations to calculate ESOP cost.</td></tr>
              ) : results.map((r:any) => (
                <tr key={r.grantId}>
                  <td style={{ fontFamily:'monospace', fontSize:12 }}>{r.grantNumber}</td>
                  <td>{r.employeeName}</td>
                  <td>{fmtC(r.exercisePrice)}</td>
                  <td>{fmtC(r.grantDateFV)}</td>
                  <td style={{ color: r.intrinsicValue>0?'var(--success)':'var(--text3)' }}>{fmtC(r.intrinsicValue)}</td>
                  <td>{fmtN(r.totalCost/Math.max(1,r.intrinsicValue))}</td>
